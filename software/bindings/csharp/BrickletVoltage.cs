...
{
	public class BrickletVoltage : Device 
	{
		private static byte TYPE_GET_VOLTAGE = 1;
		private static byte TYPE_GET_ANALOG_VALUE = 2;
		private static byte TYPE_SET_VOLTAGE_CALLBACK_PERIOD = 3;
		private static byte TYPE_GET_VOLTAGE_CALLBACK_PERIOD = 4;
		private static byte TYPE_SET_ANALOG_VALUE_CALLBACK_PERIOD = 5;
		private static byte TYPE_GET_ANALOG_VALUE_CALLBACK_PERIOD = 6;
		private static byte TYPE_SET_VOLTAGE_CALLBACK_THRESHOLD = 7;
		private static byte TYPE_GET_VOLTAGE_CALLBACK_THRESHOLD = 8;
		...
		private static byte TYPE_VOLTAGE = 13;
		private static byte TYPE_ANALOG_VALUE = 14;
		private static byte TYPE_VOLTAGE_REACHED = 15;
		private static byte TYPE_ANALOG_VALUE_REACHED = 16;

		public delegate void Voltage(ushort voltage);
		public delegate void AnalogValue(ushort value);
		public delegate void VoltageReached(ushort voltage);
		public delegate void AnalogValueReached(ushort value);

		public BrickletVoltage(string uid) : base(uid) 
		{
			messageCallbacks[TYPE_VOLTAGE] = new MessageCallback(CallbackVoltage);
			messageCallbacks[TYPE_ANALOG_VALUE] = new MessageCallback(CallbackAnalogValue);
			messageCallbacks[TYPE_VOLTAGE_REACHED] = new MessageCallback(CallbackVoltageReached);
			messageCallbacks[TYPE_ANALOG_VALUE_REACHED] = new MessageCallback(CallbackAnalogValueReached);
		}

		public void GetVoltage(out ushort voltage)
		{
			byte[] data = new byte[4];
			LEConverter.To(stackID, 0, data);
			LEConverter.To(TYPE_GET_VOLTAGE, 1, data);
			LEConverter.To((ushort)4, 2, data);

			ipcon.Write(this, data, TYPE_GET_VOLTAGE, true);

			byte[] answer;
			if(!answerQueue.TryDequeue(out answer, IPConnection.TIMEOUT_ANSWER))
			{
				throw new TimeoutException("Did not receive answer for GetVoltage in time");
			}
		...
		public void SetVoltageCallbackPeriod(uint period)
		{
			byte[] data = new byte[8];
			LEConverter.To(stackID, 0, data);
			LEConverter.To(TYPE_SET_VOLTAGE_CALLBACK_PERIOD, 1, data);
			LEConverter.To((ushort)8, 2, data);
			LEConverter.To(period, 4, data);

			ipcon.Write(this, data, TYPE_SET_VOLTAGE_CALLBACK_PERIOD, false);
		}

		public void GetVoltageCallbackPeriod(out uint period)
		{
			byte[] data = new byte[4];
			LEConverter.To(stackID, 0, data);
			LEConverter.To(TYPE_GET_VOLTAGE_CALLBACK_PERIOD, 1, data);
			LEConverter.To((ushort)4, 2, data);

			ipcon.Write(this, data, TYPE_GET_VOLTAGE_CALLBACK_PERIOD, true);

			byte[] answer;
			if(!answerQueue.TryDequeue(out answer, IPConnection.TIMEOUT_ANSWER))
			{
				throw new TimeoutException("Did not receive answer for GetVoltageCallbackPeriod in time");
			}
		...
		public void SetVoltageCallbackThreshold(char option, short min, short max)
		{
			...
			LEConverter.To(stackID, 0, data);
			LEConverter.To(TYPE_SET_VOLTAGE_CALLBACK_THRESHOLD, 1, data);
			LEConverter.To((ushort)9, 2, data);
			LEConverter.To(option, 4, data);
			LEConverter.To(min, 5, data);
			LEConverter.To(max, 7, data);

			ipcon.Write(this, data, TYPE_SET_VOLTAGE_CALLBACK_THRESHOLD, false);
		}
		...
		public void GetVoltageCallbackThreshold(out char option, out short min, out short max)
		{
			byte[] data = new byte[4];
			LEConverter.To(stackID, 0, data);
			LEConverter.To(TYPE_GET_VOLTAGE_CALLBACK_THRESHOLD, 1, data);
			LEConverter.To((ushort)4, 2, data);

			ipcon.Write(this, data, TYPE_GET_VOLTAGE_CALLBACK_THRESHOLD, true);

			byte[] answer;
			if(!answerQueue.TryDequeue(out answer, IPConnection.TIMEOUT_ANSWER))
			{
				throw new TimeoutException("Did not receive answer for GetVoltageCallbackThreshold in time");
			}
		...
		public int CallbackVoltage(byte[] data)
		{
			ushort voltage = LEConverter.UShortFrom(4, data);

			((Voltage)callbacks[TYPE_VOLTAGE])(voltage);
			return 6;
		}
		...
		public int CallbackVoltageReached(byte[] data)
		{
			ushort voltage = LEConverter.UShortFrom(4, data);

			((VoltageReached)callbacks[TYPE_VOLTAGE_REACHED])(voltage);
			return 6;
		}
		...
		{
			if(d.GetType() == typeof(Voltage))
			{
				callbacks[TYPE_VOLTAGE] = d;
			}
			else if(d.GetType() == typeof(AnalogValue))
			{
				callbacks[TYPE_ANALOG_VALUE] = d;
			}
			else if(d.GetType() == typeof(VoltageReached))
			{
				callbacks[TYPE_VOLTAGE_REACHED] = d;
			}
